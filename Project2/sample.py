...
class Word_Recognizer:
    ...
    def train_2(self, num_epochs=1):
        # sort trnlbls, endpts and trnscr such that the same word appear next to each other
        # Initialize empty lists to store sorted labels and scores
        trnlbls_sorted = []
        trnscr_sorted = []
        trnlbls_sorted_2 = []
        trnscr_sorted_2 = []
        ...
        split = int(len(self.trnscr) * 0.30)
        ...
        for i_epoch in range(num_epochs):
            ...
            self.log_likelihoods.append(log_likelihood)
            self.save(i_epoch)
            self.test_2(trnlbls_sorted_2,trnscr_sorted_2)   
    ...
    def test(self):
        ...
        for j, word in enumerate(self.train_words):
            ...
            for i, lbls in enumerate(self.devlbls):
                ...
                #print("word likeihod for i",i,"for j",j,"likelihood",word_likelihoods[j, i])
    ...
    def test_2(self,trnlbls_sorted_2,trnscr_sorted_2):
        ...
        word_likelihoods = np.zeros((len(words2id), len(trnlbls_sorted_2)))
        for j, word in enumerate(self.train_words):
            scr = []
            for letter in word:
                scr.append(self.letter2id[letter])
            ...
            for i, lbls in enumerate(trnlbls_sorted_2):
                word_likelihoods[j, i] = word_hmm.compute_log_likelihood(lbls, init_prob=init_prob, init_beta=init_beta)
                
        # Finding the most likely word for each prediction
        results = word_likelihoods.argmax(axis=0)
        predicted_words = []
        for index in results:
            predicted_words.append(id2words[index])
        ...
        for sequence in trnscr_sorted_2:
            word = ''
            for letter_id in sequence:
                word += self.id2letter[letter_id]
