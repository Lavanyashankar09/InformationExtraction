...
class HMM:
    ...
    def init_null_arcs(self, null_arcs=None):
        ...
        #print("self.null_arcs -------",self.null_arcs)
        #print("type(self.null_arcs)",type(self.null_arcs))
        ...
        # print("self.null_arcs.items()---------",self.null_arcs.items())
        # print("first element of self.null_arcs.items()---------",list(self.null_arcs.items())[0])

        for ix in self.null_arcs.keys():
            #print("self.null_arcs.keys()",self.null_arcs.keys())
            for iy in self.null_arcs[ix].keys():
                #print("self.null_arcs[ix].keys()",self.null_arcs[ix].keys())
        ...
        while len(stack) > 0:
            ...
            if s not in self.null_arcs:
                # print("s in not in  ",s)
                # print("self.null_arcs in not in ",self.null_arcs)
                continue
            for s_y in self.null_arcs[s]:
                # print("self.null_arcs[s]",self.null_arcs[s])
                # print("s_y",s_y)
                count[s_y] -= 1
                # print("count[s_y]",count[s_y])
                # print("count ------------------------",count)
                if count[s_y] == 0:
                    stack.append(s_y)
    ...
    def forward(self, data, init_prob=None):
        ...
        for t in range(1, len(data) + 1):
            ...
            # non_null arcs
            # print("alphas_[t-1]", alphas_[t - 1])
            # print("self.transitions", self.transitions)
            # print("self.emissions[obs]", self.emissions[obs])
            # print("self.transitions * self.emissions[obs]", self.transitions * self.emissions[obs])

            alphas_[t] = np.dot(alphas_[t - 1], self.transitions * self.emissions[obs])


            # print("shape of alphas_[t]", alphas_[t].shape)
            # print("alphas_[t] with out", alphas_[t])
